...
BOOST_AUTO_TEST_CASE(addrman_simple)
{
    ...
    BOOST_CHECK_EQUAL(addr_ret1.ToStringAddrPort(), "250.1.1.1:29333");
    ...
    vAddr.emplace_back(ResolveService("250.1.1.3", 29333), NODE_NONE);
    vAddr.emplace_back(ResolveService("250.1.1.4", 29333), NODE_NONE);
...
BOOST_AUTO_TEST_CASE(addrman_ports)
{
    ...
    CService addr1_port = ResolveService("250.1.1.1", 29334);
    BOOST_CHECK(addrman->Add({CAddress(addr1_port, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 2U);
    auto addr_ret2 = addrman->Select().first;
    BOOST_CHECK(addr_ret2.ToStringAddrPort() == "250.1.1.1:29333" || addr_ret2.ToStringAddrPort() == "250.1.1.1:29334");
    ...
    BOOST_CHECK_EQUAL(addr_ret3.ToStringAddrPort(), "250.1.1.1:29333");
}
...
BOOST_AUTO_TEST_CASE(addrman_select_special)
{
    ...
    // add ipv4 address to the new table
    CService addr1 = ResolveService("250.1.1.3", 29333);
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
...
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CAddress addr{CAddress(ResolveService("253.3.3.3", 29333), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(load_addrman)
{
    ...
    addr1 = Lookup("250.7.1.1", 29333, false);
    ...
    const std::optional<CService> source{Lookup("252.5.1.1", 29333, false)};
...
BOOST_AUTO_TEST_CASE(addrman_update_address)
{
    ...
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CNetAddr source{ResolveIP("252.2.2.2")};
    CAddress addr{CAddress(ResolveService("250.1.1.1", 29333), NODE_NONE)};
    ...
    CAddress addr_diff_port{CAddress(ResolveService("250.1.1.1", 29334), NODE_NONE)};
    ...
    CAddress addr_v2{CAddress(ResolveService("250.1.1.1", 29333), NODE_P2P_V2)};
...
BOOST_AUTO_TEST_CASE(addrman_size)
{
    ...
    const CAddress addr1{ResolveService("250.1.1.1", 29333), NODE_NONE};
    BOOST_CHECK(addrman->Add({addr1}, source));
    BOOST_CHECK(addrman->Good(addr1));
    const CAddress addr2{ResolveService("250.1.1.2", 29333), NODE_NONE};
